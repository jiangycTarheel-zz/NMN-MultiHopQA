...
def get_2d_spans(text, tokenss, bert=False):
  spanss = []
  cur_idx = 0
  for tokens in tokenss:
    ...
    for token in tokens:
      ...
      if text.find(token, cur_idx) < 0:
        if bert:
          _cur_idx = text.find(token[0], cur_idx)
          if _cur_idx < 0:
            print(tokens)
            print("{} {} {}".format(token, cur_idx, text))
            print(cur_idx)
            print(text[cur_idx:cur_idx+10])
            #print("Text: " + text)
            raise Exception()
          else:
            cur_idx = _cur_idx
...
def get_word_span(context, wordss, start, stop, bert=False):
  spanss = get_2d_spans(context, wordss, bert=bert)
  idxs = []
...
def get_word_idx(context, wordss, idx, bert=False):
  spanss = get_2d_spans(context, wordss, bert)
...
def get_best_candidate(ypi, cand_spans):
  max_val = 0
  best_word_span = (0, 1)
  best_sent_idx = 0
  for f, ypif in enumerate(ypi):
    #assert len(cand_spans) == len(ypif), (len(cand_spans), len(ypif))
    argmax_j1 = 0
    #assert len(ypif) == len(cand_span), (len(ypif), len(cand_span))
    for j in range(len(cand_spans)):
      val1 = ypif[argmax_j1]
      if val1 < ypif[j]:
        val1 = ypif[j]
        argmax_j1 = j
      ...
      if val1 > max_val:
        try:
          best_word_span = (cand_spans[argmax_j1][0][1], cand_spans[argmax_j1][1][1])
        except IndexError:
          print(cand_spans)
          print(argmax_j1)
        best_sent_idx = f
        max_val = val1
  return ((best_sent_idx, best_word_span[0]), (best_sent_idx, best_word_span[1])), float(max_val)
