...
def dense(x, outdim, activation=None, scope=None, reuse=False):
  with tf.variable_scope(scope if scope else '', reuse=reuse):
    out = tf.layers.dense(x, outdim, activation=activation)
  return out
